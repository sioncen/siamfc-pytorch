...
class Pair(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if randp<0.05:
            for i in range(100):
                box_xT = anno[rand_z+1]
                wb=int(random.uniform(0.2,0.4)*box_xT[2])
                hb=int(random.uniform(0.2,0.4)*box_xT[3])
                xb=int(box_xT[0]-wb)
                yb=int(random.uniform(box_xT[1],box_xT[1]+box_xT[3]-hb))
                if xb>0 and yb>0 and (yb+hb)<x.shape[1]:
                    bflag=True
                    break
            if bflag:
                box_x = anno[rand_z+1]
                x = cv2.imread(img_files[rand_z+1], cv2.IMREAD_COLOR)
                x = cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
                x[yb:yb+hb, xb+wb:xb+2*wb] = x[yb:yb+hb, xb:xb+wb]
            else:
                print("aaaaaaaaaaaaaaaaaaaaaaaaa")
        elif randp<0.1:
            for i in range(100):
                box_xT = anno[rand_z+1]
                wb = int(random.uniform(0.2, 0.5) * box_xT[2])
                hb = int(random.uniform(0.2, 0.5) * box_xT[3])
                xb = int(box_xT[0] + box_xT[2])
                yb = int(random.uniform(box_xT[1], box_xT[1] + box_xT[3] - hb))
                if (xb+wb) < x.shape[1] and yb > 0 and (yb + hb)<x.shape[1]:
                    bflag=True
                    break
            if bflag:
                box_x = anno[rand_z+1]
                x = cv2.imread(img_files[rand_z+1], cv2.IMREAD_COLOR)
                x = cv2.cvtColor(x, cv2.COLOR_BGR2RGB)
                x[yb:yb + hb, xb - wb:xb] = x[yb:yb + hb, xb:xb + wb]
            else:
